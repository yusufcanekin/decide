...
public class Cmv {
    ...
    public static boolean[] computeCMV() {
        ...
        // Defensive defaults (prevents NullPointerExceptions in LICs)
        ...
        if (Main.X == null || Main.Y == null) {
            Main.NUMPOINTS = 0;
            return cmv; // all false
        }

        // Make NUMPOINTS consistent with the actual data
        int n = Math.min(Main.X.length, Main.Y.length);

        // If NUMPOINTS is not set or is inconsistent, clamp it safely
        if (Main.NUMPOINTS <= 0 || Main.NUMPOINTS > n) {
            Main.NUMPOINTS = n;
        }

        // Now it is safe to evaluate all LICs
        cmv[0]  = lic0();
        cmv[1]  = lic1();
        cmv[2]  = lic2();
        cmv[3]  = lic3();
